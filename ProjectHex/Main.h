...
void cache()
{
	while (true)
	{
		...
		camera_manager = Driver::read<uint64_t>(pid, playercontroller + 0x428);
		...
		for (unsigned long i = 0; i < actor_count; ++i)
		{
			...
			//uint64_t mesh = Driver::read<uint64_t>(pid, actor + 0x3E0); SECOND ONE
			uint64_t mesh = Driver::read<uint64_t>(pid, actor + 0x518);
			...
			else if (MenuTab == 1)
			{
				if (VisualTab == 0)
				{
					...
					ImGui::BeginChild("##Players22", ImVec2(-1, 175.f), true);
					{
						...
						ImGui::SliderFloat("##BoxThick", &menu::box_thick, 0.f, 15.f);
						ImGui::PopItemWidth();
...
DWORD GetProcessIdByName(wchar_t* name) {
	...
	if (Process32First(snapshot, &entry) == TRUE) {
		while (Process32Next(snapshot, &entry) == TRUE) {
			if (_wcsicmp((const wchar_t*)entry.szExeFile, name) == 0) {
